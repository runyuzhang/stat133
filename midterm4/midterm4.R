...
dice_sum <- function(k=2, B=100){
  dsum = 1:B
  dsum = sapply(dsum, function(x) sum(sample(1:6, k, replace = TRUE)))
  return(dsum)
}
...
even <- paste(rep("even", 1000), (1:500)*2, sep = "")
...
updateDate <- function(dates, old.yr) {
  valid.dates = dates[grep(old.yr, dates)]
  updated.dates = gsub(old.yr, "2015", valid.dates)
